package edu.miu.waa.minimartecommerce.exceptionHandling.advice;

import edu.miu.waa.minimartecommerce.exceptionHandling.dto.ApiError;
...
import edu.miu.waa.minimartecommerce.exceptionHandling.exceptions.ExtensionMismatchException;
...
public class ExceptionHandlingControllerAdvice extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleAuthenticationException(final Exception ex) {
        logger.error(ex.getClass().getName());
        logger.error(ex.getMessage());
        final ApiError apiError = new ApiError(HttpStatus.FORBIDDEN, ex.getMessage(),  ex.getMessage());
        return new ResponseEntity<>(apiError, new HttpHeaders(), apiError.getStatus());
    }

    @ExceptionHandler({AccountNotActivatedException.class})
    public ResponseEntity<ApiError> handleInactiveAccountException(final Exception ex) {
        logger.error(ex.getClass().getName());
        logger.error(ex.getMessage());
        final ApiError apiError = new ApiError(HttpStatus.NOT_ACCEPTABLE, ex.getMessage(),  ex.getMessage());
        return new ResponseEntity<>(apiError, new HttpHeaders(), apiError.getStatus());
    }
